...
const LEDBoard = ({ initialData }: { initialData: any }) => {
  ...
  const CourtDisplay = ({ side, courtData, currentTimer }: any) => {
    ...
    return (
      <div
        className={`absolute top-1/2 -translate-y-1/2 ${heightClass} ${widthClass} ${boxStyle} flex items-center justify-center px-4 overflow-hidden border-b-4 border-[#E38035] ${posClass}`}
      >
        <div className="flex items-center justify-between w-full relative h-full gap-2">
          <span className="text-5xl text-[#FFF6E5] text-center z-10 w-[80px] font-mono leading-none font-bold">
            {courtData.p1}
          </span>
          <span className="text-[#E38035] text-[5rem] leading-none z-10 font-sans font-thin italic transform -skew-x-12 opacity-80">
            /
          </span>
          <span className="text-5xl text-[#FFF6E5] text-center z-10 w-[80px] font-mono leading-none font-bold">
            {courtData.p2}
