...
const ControlPanel = () => {
  ...
  useEffect(() => {
    ...
    state.left.timerStored,
    state.right.timerStart,
    state.right.timerStored,
  ]);
  ...
  const handleContinue = (courtSide) => {
    updateState((prev) => {
      ...
      const c = n[courtSide];
      if (c.winner === 1) c.p1Sets++;
      else if (c.winner === 2) c.p2Sets++;
      c.winner = null;
      c.p1 = 0;
      c.p2 = 0;
      return n;
  ...
  const updateSet = (courtSide, player, delta) => {
    updateState((prev) => {
      const n = JSON.parse(JSON.stringify(prev));
      if (player === 1)
        n[courtSide].p1Sets = Math.max(0, n[courtSide].p1Sets + delta);
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto font-sans min-h-screen pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-gray-900 p-6 rounded-2xl border border-gray-800">
            <div className="flex justify-between items-center mb-4">
              <h1 className="text-3xl font-bold text-[#E38035]">
                PADEL CONTROL
              </h1>
              <button
                onClick={() =>
          ...
          <div className="bg-gray-900 p-6 rounded-2xl border border-gray-800 flex flex-col gap-4">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (f) {
                        ...
                        r.onloadend = () =>
                          updateState((s) => ({ ...s, logoSrc: r.result }));
                        r.readAsDataURL(f);
                      }
                    }}
                  />
            ...
            {state.motion.src && (
              <div className="flex flex-col gap-2">
                ...
                >
                  {state.motion.active ? "STOP MOTION" : "PLAY MOTION"}
                </button>
                <button
                  onClick={() =>
                    updateState(
        ...
        <div className="mb-6 bg-gray-900 p-4 rounded-xl border-l-4 border-[#E38035]">
          <div className="flex justify-between items-center">
            <h1 className="text-2xl font-bold text-white uppercase">
              REMOTE: {filterCourt} COURT
            </h1>
            <button
              onClick={() => (window.location.href = "?view=control")}
          ...
          <div className="grid grid-cols-3 gap-2 mt-4 bg-black/50 p-1 rounded-lg">
            {[
              { id: "simple", icon: LayoutTemplate, label: "Score" },
              { id: "timer", icon: Clock, label: "Timer" },
              { id: "full", icon: Trophy, label: "Detail" },
            ].map((m) => (
              <button
                key={m.id}
                onClick={() =>
      ...
      {/* COURTS GRID */}
      <div
        className={`grid grid-cols-1 ${!filterCourt ? "lg:grid-cols-2" : ""} gap-8 ${state.template === "text" || state.motion.active ? "opacity-40 pointer-events-none grayscale" : ""}`}
      >
        {["left", "right"].map((side) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-neutral-950 gap-10 font-sans text-white p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl w-full">
        ...
        >
          <span className="block text-2xl font-bold mb-2 text-white">
            <Settings2 className="inline mr-2" /> Full Master Controller
          </span>
          <span className="block text-sm text-gray-400">
            Control everything (Left + Right)
          </span>
        </button>
