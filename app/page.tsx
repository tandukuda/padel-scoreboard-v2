...
import { Monitor, Settings2, Play, Pause, RotateCcw, Clock, Type, LayoutTemplate, Trophy, Video, Image as ImageIcon, ArrowRight, Eye, EyeOff, Smartphone, PlayCircle } from "lucide-react";
...
const LEDBoard = ({ initialData }) => {
    ...
    if (courtData.winner) {
      return (
        <div className={`absolute top-1/2 -translate-y-1/2 h-[84px] min-w-[260px] bg-[#0C0C0C] rounded-2xl flex items-center justify-center px-6 shadow-lg ${posClass}`}>
           <span className="text-4xl text-[#E38035] animate-pulse tracking-wider font-mono">WINNER</span>
        </div>
      );
    }
      ...
      return (
        <div className={`absolute top-1/2 -translate-y-1/2 flex items-center gap-4 ${posClass} ${!isLeft ? 'flex-row-reverse' : ''}`}>
           <div className="h-[84px] min-w-[160px] bg-[#0C0C0C] rounded-2xl flex items-center justify-center px-4 shadow-lg border-l-4 border-[#E38035]">
              <span className="text-5xl text-[#FFF6E5] tracking-widest font-mono">{formatTimer(currentTimer)}</span>
           </div>
           ...
           <div className={`flex flex-col justify-center h-full flex-1 min-w-0 ${borderClass} border-gray-800`}>
              ...
              <div className={`flex items-center ${dirClass} justify-between h-1/2 px-4 border-b border-gray-900`}>
                 <div className="overflow-hidden w-full relative">
                    <span className={`text-xl text-white uppercase block truncate ${textAlign} font-mono`}>{courtData.p1Name}</span>
                 </div>
                 <div className="bg-gray-800 px-2 rounded mx-2 min-w-[30px] text-center">
                    <span className="text-[#E38035] font-mono">{courtData.p1Sets}</span>
                 </div>
              </div>
              {/* Player 2 Name & Set */}
              <div className={`flex items-center ${dirClass} justify-between h-1/2 px-4`}>
                 <div className="overflow-hidden w-full relative">
                    <span className={`text-xl text-white uppercase block truncate ${textAlign} font-mono`}>{courtData.p2Name}</span>
                 </div>
                  <div className="bg-gray-800 px-2 rounded mx-2 min-w-[30px] text-center">
                    <span className="text-[#E38035] font-mono">{courtData.p2Sets}</span>
                  </div>
              </div>
           </div>
           {/* Scores */}
           <div className="flex flex-col justify-center h-full px-2 w-[70px] bg-black items-center">
               <span className="text-3xl text-[#FFF6E5] leading-tight mb-1 font-mono">{POINTS[courtData.p1]}</span>
               <span className="text-3xl text-[#FFF6E5] leading-tight font-mono">{POINTS[courtData.p2]}</span>
           </div>
        </div>
      );
    }

    // TEMPLATE 1: SIMPLE (Montserrat Slash)
    return (
      <div className={`absolute top-1/2 -translate-y-1/2 h-[84px] min-w-[260px] bg-[#0C0C0C] rounded-2xl flex items-center justify-center px-4 shadow-lg overflow-hidden ${posClass}`}>
          <div className="flex items-center justify-between w-full relative h-full">
            <span className="text-6xl text-[#FFF6E5] text-center z-10 w-[60px] font-mono">{POINTS[courtData.p1]}</span>

            {/* THE SLASH: Montserrat (Sans), Thin (100), Big */}
            <span className="text-[#E38035] italic text-[220px] leading-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-[60%] transform -skew-x-12 z-0 select-none font-sans font-extralight">
              /
            </span>

            <span className="text-6xl text-[#FFF6E5] text-center z-10 w-[60px] font-mono">{POINTS[courtData.p2]}</span>
          </div>
      </div>
    );
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black overflow-hidden font-mono">
      <style>{`
             ...
             <div className="absolute inset-0 z-50 bg-black flex items-center justify-center overflow-hidden rounded-[30px]">
                ...
                {data.motion.showClock && (
                   <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-[84px] bg-black/60 backdrop-blur-md rounded-2xl flex items-center justify-center px-10 border border-white/20">
                      <span className="text-6xl text-white tracking-widest font-mono">{time}</span>
                   </div>
                )}
             </div>
          )}
                ...
                {data.template === "text" ? (
                   <>
                     <div className="absolute inset-0 flex items-center overflow-hidden">
                        <div className={`whitespace-nowrap text-[70px] text-black uppercase font-bold font-mono ${data.isAnimating ? 'animate-marquee' : ''}`}>
                          {loopedText}
                        </div>
                     </div>
                     {data.showClock && (
                       <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20">
                         <div className="bg-[#0C0C0C] text-[#FFF6E5] rounded-xl px-8 py-2 shadow-2xl border-2 border-[#E38035]">
                           <span className="text-5xl tracking-widest font-mono">{time}</span>
                         </div>
                      </div>
                     )}
                   </>
                   ...
                   <>
                      ...
                      {data.showClock && (
                        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-[84px] bg-[#0C0C0C] rounded-2xl flex items-center justify-center px-10 shadow-xl border border-gray-800">
                          <span className="text-6xl text-[#FFF6E5] tracking-widest mt-1 font-mono">{time}</span>
                        </div>
                      )}
...
// --- COMPONENT: CONTROLLER (Uses Default Sans = Montserrat) ---
const ControlPanel = () => {
  // SET DEFAULT: Motion Active = true, Source = /intro.mp4
  const [state, setState] = useState({
    ...
    showClock: true,
    logoSrc: null,
    motion: { src: '/intro.mp4', active: true, showClock: true }, // <--- AUTO STARTUP
  ...
  useEffect(() => {
    ...
    const poll = setInterval(async () => {
        try {
            ...
            // Deep merge logic simplified for polling
            setState(prev => {
                // If the server says motion is active, we respect it
                return {...prev, ...data};
            });
        } catch(e) {}
    }, 1000);
  ...
  // ... (Keep existing game logic functions: handleContinue, handlePoint, toggleTimer, etc.)
  ...
  const handleVideoUpload = (e) => {
      ...
      if (file) {
        ...
        // AUTO PLAY ON UPLOAD
        reader.onloadend = () => updateState(s => ({ ...s, motion: { ...s.motion, src: reader.result, active: true } }), 'motion');
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto font-sans min-h-screen pb-20">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        {/* BIG START MATCH BUTTON (Only shows if Motion is Active) */}
        {state.motion.active && (
            <button
                onClick={() => updateState(s => ({...s, motion: {...s.motion, active: false}}), 'motion')}
                className="bg-green-600 hover:bg-green-500 text-white px-8 py-4 rounded-xl font-bold text-2xl shadow-xl animate-pulse flex items-center gap-3"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-gray-900 p-6 rounded-2xl border border-gray-800">
            <div className="flex justify-between items-center mb-4">
               <h2 className="text-gray-400 font-bold">BOARD LAYOUT</h2>
          ...
          <div className="bg-gray-900 p-6 rounded-2xl border border-gray-800 flex flex-col gap-4">
             <div className="flex gap-4 items-center">
                ...
                <div className="flex-1 bg-black p-2 rounded border border-gray-700 flex items-center gap-2">
                   ...
                   <label className="text-xs text-gray-400 font-bold flex-1 cursor-pointer">MOTION<input type="file" accept="video/*" className="hidden" onChange={handleVideoUpload}/></label>
                </div>
             </div>
             ...
             <div className="flex flex-col gap-2">
                {!state.motion.active && (
                    <button onClick={() => updateState(s => ({...s, motion: {...s.motion, active: true}}), 'motion')} className="w-full py-2 rounded font-bold text-xs bg-gray-700 hover:bg-gray-600 text-gray-300">
                        PLAY INTRO LOOP
                    </button>
                )}
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-neutral-950 gap-10 font-sans text-white p-4">
      <div className="text-center animate-in fade-in slide-in-from-bottom-4 duration-700">
        <h1 className="text-5xl font-extrabold text-[#E38035] mb-2 tracking-tight">PADEL SYSTEM v2.5</h1>
